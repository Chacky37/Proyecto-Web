...
$con = mysqli_connect($host, $user, $pass, $db);
    ...
    if (isset($_POST['actualizar'])) {
        ...
        if(isset($_FILES['image'])){
            ...
            if ( ($megabyte>$size_array[0])&&($megabyte>$size_array[1])&&($megabyte>$size_array[2])&&($megabyte>$size_array[3]) ) {
                ...
                if ( in_array($imageType[0], $Extension) && in_array($imageType[1], $Extension) && in_array($imageType[2], $Extension) && in_array($imageType[3], $Extension)  ){
                    ...
                    $actualizarproducto ="  UPDATE productos  SET nombre='$nombre', precio='$precio', sku='$sku', disponibilidad='$disponibilidad', talla='$talla', etiqueta='$etiqueta', 
                     seccion='$seccion', categoria='$categoria', imagen1='$name_array[0]', imagen2='$name_array[1]', imagen3='$name_array[2]', imagen4='$name_array[4]' WHERE id = $idproducto ";
            
                    $res = mysqli_query($conexion, $actualizarproducto);
                    #print_r($insertar);
                    ...
                    header("location: app-products-list.php?modulo=productos&notificacion=¡Error!..... la imagenes tienen que ser : jpg, jpeg, png");
                    //print_r($imageType);
                }
        
            }else{
                header("location: app-products-list.php?modulo=productos&notificacion=¡Error!..... las imagenes tienen que ser menor a un tamaño de 1MB");
        ...
        if ($res) {
            header("location: app-products-list.php?modulo=productos&mensaje=¡Producto registrado con exito!");
